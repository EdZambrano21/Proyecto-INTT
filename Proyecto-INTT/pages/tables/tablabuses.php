...
if (!isset($_SESSION['user_id'])) {
  ...
  //Impresión de tabla
   ...
   $query = "SELECT * FROM tmaveh"; // Reemplaza 'nombre_de_tabla' con el nombre real de tu tabla
  ...
  <body class="skin-blue">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Tabla de Vehículos</li>
          </ol>
        </section>
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<td>{$row['vehpla']}</td>";
                        echo "<td>{$row['vehano']}</td>";
                        echo "<td>{$row['vehest']}</td>";
                        echo "<td>{$row['proced']}</td>";
                        echo "<td>{$row['cupide']}</td>";
                        echo "<td>{$row['modide']}</td>";
                        echo "<td>{$row['maride']}</td>";
                        echo "<td><button class='btn btn-primary editar-btn' data-toggle='modal' data-target='#editarModal' data-id='{$row['vehcha']}'>Editar</button></td>";
                        echo "</tr>";
                      }
    ...
    $(document).ready(function () {
        ...
        $('.editar-btn').click(function () {
            ...
            $.ajax({
                url: 'obtener_vehiculo.php', // Reemplaza con el nombre de tu archivo PHP para obtener datos del vehículo
                method: 'POST',
                data: { vehiculoId: vehiculoId },
                dataType: 'json',
                success: function (data) {
                    // Rellena el formulario de edición con los datos del vehículo
                    $('#vehiculoId').val(data.vehcha);
                    $('#nuevoCampo1').val(data.vehest);
        ...
        $('#editarForm').submit(function (e) {
            ...
            $.ajax({
                ...
                error: function () {
                    alert('Error al actualizar datos del vehículo');
                }
            });
        });
    });
